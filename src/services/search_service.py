    ...
    def search_sns_content(
        ...
        for idx, (tbs_value, period_name) in enumerate(time_ranges, 1):
            try:
                ...
                log(
                    self.__class__.__name__,
                    f"SNS 검색 Debug - Google Images 후보: {len(images_candidates)}개",
                )
                log(
                    self.__class__.__name__,
                    f"SNS 검색 Debug - YouTube Direct 후보: {len(youtube_candidates)}개",
                )
                ...
                log(
                    self.__class__.__name__,
                    f"SNS 검색 Debug - 병합 후 고유 후보: {len(unique_candidates)}개",
                )
                ...
                for idx, candidate in enumerate(unique_candidates):
                    ...
                    log(
                        self.__class__.__name__,
                        f"SNS 검색 Debug - Candidate #{idx}: platform={platform}, url={url[:80] if url else 'None'}, source={source}",
                    )
                    ...
                    if relevance_checker and user_question:
                        ...
                        if is_relevant:
                            log(
                                self.__class__.__name__,
                                f"SNS 검색 Debug - ✅ 관련성 확인 완료 ({period_name}) → 결과 반환",
                            )
                            return {
                                "found": True,
                                "platform": platform,
                            ...
                            log(
                                self.__class__.__name__,
                                f"SNS 검색 Debug - ❌ 관련성 없음 → 다음 결과 탐색",
                            )
                            continue
                    else:
                        log(
                            self.__class__.__name__,
                            f"SNS 검색 Debug - ✅ 관련성 검증 없이 결과 반환 ({period_name})",
                        )
                        return {
                            "found": True,
                            "platform": platform,
                ...
                log(
                    self.__class__.__name__,
                    f"SNS 검색 Debug - ❌ {period_name}에서 SNS 링크를 찾지 못함 → Fallback 시도",
                )
                ...
                if idx < len(time_ranges):
                    next_period = time_ranges[idx][1]
                    log(
                        self.__class__.__name__, f"오류 발생 - 다음 시도: {next_period}"
                    )
                continue
