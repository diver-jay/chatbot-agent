...
class GeneralSearchStrategy(SearchStrategy):
    ...
    def _extract_summary(self, search_results: Dict[str, Any]) -> str:
        ...
        if "answer_box" in search_results and search_results["answer_box"].get("answer"):
        ...
        if "organic_results" in search_results and search_results["organic_results"]:
            return "\n".join([f"• {r.get('snippet', '')}" for r in search_results["organic_results"][:3]])
        return "검색 결과를 찾을 수 없습니다."


class TermSearchStrategy(SearchStrategy):
    """신조어/용어 검색을 위한 독립적인 전략"""
...
class SnsSearchStrategy(SearchStrategy):
    """SNS 콘텐츠를 검색하고, 실패 시 일반 검색으로 fallback하는 독립적인 전략"""

    def __init__(self, serpapi_key: str, youtube_api_key: str, relevance_checker, **kwargs):
        self.api_key = serpapi_key
        self.youtube_api_key = youtube_api_key
        self.relevance_checker = relevance_checker
        self.base_url = "https://serpapi.com/search"
        self.youtube_base_url = "https://www.googleapis.com/youtube/v3/search"
        self._fallback_strategy = GeneralSearchStrategy(serpapi_key=serpapi_key)
    ...
    def search(self, query: str, question: str) -> Tuple[str, Optional[Dict[str, Any]]]:
        ...
        if sns_content and sns_content.get("found"):
            ...
            return self._fallback_strategy.search(query, question)

    def _search_sns_content(self, query: str, user_question: str = "") -> Dict[str, Any]:
        ...
        for candidate in sorted_candidates:
            if self.relevance_checker.act(user_question=user_question, sns_title=candidate.get("title", ""), platform=candidate.get("platform", ""), search_term=query):
                return {"found": True, **candidate}
        return {"found": False}

    def _clean_youtube_query(self, query: str) -> str:
        for keyword in ["유튜브", "youtube", "영상", "동영상", "비디오", "video"]:
            query = query.replace(keyword, "").replace(keyword.upper(), "").replace(keyword.capitalize(), "")
        return " ".join(query.split())

    def _search_all_time_ranges_parallel(self, query: str, youtube_query: str, time_ranges: List[tuple]) -> List[Dict[str, Any]]:
        all_candidates = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = []
            for tbs, period in time_ranges:
                futures.append(executor.submit(self._search_google_images_with_metadata, query, tbs, period))
                if self.youtube_api_key:
                    futures.append(executor.submit(self._search_youtube_direct_with_metadata, youtube_query, self._get_youtube_time_filter(tbs), period))
            
            for future in concurrent.futures.as_completed(futures):
                all_candidates.extend(future.result())

        seen_urls = set()
        unique_candidates = [c for c in all_candidates if c.get("url") and c["url"] not in seen_urls and not seen_urls.add(c["url"])]
        return unique_candidates

    def _search_google_images_with_metadata(self, query: str, tbs_value: Optional[str], period_name: str) -> List[Dict[str, Any]]:
        candidates = self._search_google_images(query, tbs_value)
        for c in candidates: c.update({"time_range_priority": period_name, "time_range_tbs": tbs_value})
        return candidates

    def _search_youtube_direct_with_metadata(self, query: str, published_after: Optional[str], period_name: str) -> List[Dict[str, Any]]:
        candidates = self._search_youtube_direct(query, published_after)
        for c in candidates: c.update({"time_range_priority": period_name, "time_range_published_after": published_after})
        return candidates

    def _sort_by_recency(self, candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        order = {"최근 3개월": 0, "최근 6개월": 1, "최근 1년": 2, "전체 기간": 3}
        return sorted(candidates, key=lambda c: order.get(c.get("time_range_priority", "전체 기간"), 99))

    def _search_google_images(self, query: str, tbs_value: Optional[str]) -> List[Dict[str, Any]]:
        params = {"q": query, "api_key": self.api_key, "engine": "google_images", "num": 50, "hl": "ko", "gl": "kr"}
        if tbs_value: params["tbs"] = tbs_value
        try:
            ...
            for res in results:
                ...
                if "instagram.com" in link and ("/p/" in link or "/reel/" in link):
                    candidates.append({"platform": "instagram", "url": link, "thumbnail": res.get("thumbnail"), "title": res.get("title"), "source": "google_images"})
                elif "youtube.com/watch" in link or "youtu.be/" in link:
                    candidates.append({"platform": "youtube", "url": link, "thumbnail": res.get("thumbnail"), "title": res.get("title"), "source": "google_images"})
    ...
    def _search_youtube_direct(self, query: str, published_after: Optional[str]) -> List[Dict[str, Any]]:
        ...
        params = {"part": "snippet", "q": query, "key": self.youtube_api_key, "type": "video", "maxResults": 5, "regionCode": "KR", "relevanceLanguage": "ko", "order": "relevance"}
    ...
    def _get_youtube_time_filter(self, tbs_value: Optional[str]) -> Optional[str]:
        if not tbs_value: return None
        days = {"qdr:m3": 90, "qdr:m6": 180, "qdr:y": 365}.get(tbs_value)
        return (datetime.utcnow() - timedelta(days=days)).strftime("%Y-%m-%dT%H:%M:%SZ") if days else None
